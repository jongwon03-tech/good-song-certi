      ...
      <header className="bg-white border-b sticky top-0 z-50 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg">굿모닝송도 <span className="text-orange-600">기록증</span></h1>
          </div>
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button onClick={() => setActiveTab('edit')} className={`px-4 py-1.5 rounded-md text-sm font-bold ${activeTab === 'edit' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500'}`}>기록 입력</button>
            <button onClick={() => setActiveTab('log')} className={`px-4 py-1.5 rounded-md text-sm font-bold ${activeTab === 'log' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500'}`}>로그 ({history.length})</button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md border border-slate-200 min-h-[400px]">
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => { localStorage.removeItem('goodsong_history'); setHistory([]); }} className="w-full py-3 text-red-500 text-sm font-medium">로그 모두 삭제</button>
              </div>
            )}
          </div>
        )}
        ...
        <section id="preview-section" className="space-y-6">
          ...
          <div className="bg-slate-200 rounded-3xl p-6 flex items-center justify-center overflow-hidden min-h-[500px] border-4 border-dashed border-slate-300 relative">
            <div className="scale-[0.4] sm:scale-[0.5] md:scale-[0.6] lg:scale-[0.7] origin-center transition-transform">
